...
const AdminClientDetails = ({ paciente, onBack, onRefreshData, globalMedicamentos, setModalState, toast }) => {
  ...
  const exportClientDataToPDF = () => {
    ...
    doc.text(`Informe Paciente: ${paciente.username}`, 14, 22);

    doc.setFontSize(11);
    doc.setTextColor(100, 116, 139);
    doc.text(`Email: ${paciente.email}`, 14, 30);
    doc.text(`ID: ${paciente.id}`, 14, 36);
    ...
    addSectionToPdf("Medicamentos", 
      ...
      paciente.medicamentos, "No hay medicamentos registrados."
    );
    ...
    addSectionToPdf("Recordatorios",
      ...
      paciente.recordatorios, "No hay recordatorios registrados."
    );
    
    const inventarioData = paciente.inventario?.map(item => {
        const med = paciente.medicamentos?.find(m => m.id === item.medicamentoId);
    ...
    addSectionToPdf("Interacciones",
      ...
      paciente.interacciones, "No hay interacciones registradas."
    );
    ...
    addSectionToPdf("Historial de Tomas",
      ...
      paciente.historial, "No hay historial de tomas."
    );

    doc.save(`Informe_RecordaMedic_${paciente.username.replace(/\s/g, '_')}.pdf`);
    toast({ title: "PDF Exportado", description: "El informe del paciente ha sido generado.", className: "bg-green-500 text-white" });
  };
  ...
  const tabItems = [
    { value: "medicamentos", label: "Medicamentos", icon: PillIcon, data: paciente.medicamentos, cols: [
    ...
    { value: "recordatorios", label: "Recordatorios", icon: BellIcon, data: paciente.recordatorios, cols: [
    ...
    { value: "inventario", label: "Inventario", icon: ArchiveIcon, data: paciente.inventario?.map(i => ({...i, medicamentoNombre: globalMedicamentos.find(m => m.id === i.medicamentoId)?.nombre || 'N/A' })), cols: [
    ...
    { value: "interacciones", label: "Interacciones", icon: AlertTriangleIcon, data: paciente.interacciones, cols: [
    ...
    { value: "historial", label: "Historial", icon: HistoryIcon, data: paciente.historial, cols: [
    ...
    >
      <div className="flex flex-wrap justify-between items-center gap-4 mb-2">
        <Button onClick={onBack} variant="outline" className="shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-slate-800">
          <ArrowLeft className="mr-2 h-4 w-4" /> Volver a Gestión de Pacientes
        </Button>
      ...
      <Card className="shadow-2xl rounded-xl border-0 overflow-hidden bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg">
        <CardHeader className="bg-gradient-to-br from-primary via-sky-500 to-indigo-600 text-white p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-3xl md:text-4xl font-bold">{paciente.username}</CardTitle>
              ...
              {tabItems.map(tab => (
                <TabsContent key={tab.value} value={tab.value}>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl md:text-2xl font-semibold text-slate-700 dark:text-slate-200">{tab.label} del Paciente</h3>
                  ...
                  ) : <p className="text-muted-foreground text-center py-8 text-lg">No hay {tab.label.toLowerCase()} registrados para este paciente.</p>}
                </TabsContent>
              ))}
            </div>
          </Tabs>
