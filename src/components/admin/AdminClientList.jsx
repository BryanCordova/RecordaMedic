...
const AdminClientList = ({ pacientes, onSelectPaciente, onDeletePaciente, searchTerm }) => {
  if (pacientes.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-sm">
          {searchTerm ? "Intenta con otros términos de búsqueda." : "Los nuevos pacientes aparecerán aquí."}
        </p>
      </motion.div>
    );
  }
            ...
            <CardContent className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="flex-grow">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <div>
                    <p className="font-bold text-lg text-slate-800 dark:text-slate-100">{paciente.username}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{paciente.email}</p>
                  </div>
                </div>
                <div className="text-xs mt-3 flex flex-wrap gap-2">
                  <ClientStat type="meds" count={paciente.medicamentos?.length} />
                  <ClientStat type="record" count={paciente.recordatorios?.length} />
                  <ClientStat type="inv" count={paciente.inventario?.length} />
                  <ClientStat type="interac" count={paciente.interacciones?.length} />
                  <ClientStat type="hist" count={paciente.historial?.length} />
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 mt-3 sm:mt-0 w-full sm:w-auto">
                ...
                <Button 
                  variant="destructive" 
                  size="sm" 
                  onClick={() => onDeletePaciente(paciente)}
