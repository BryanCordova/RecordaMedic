...
const AuthForm = ({ mode, role, isOpen, onClose, onSuccess }) => {
  ...
  const title = `${isRegisterMode ? 'Crear Cuenta' : 'Iniciar Sesión'} ${role === 'admin' ? 'de Administrador' : 'de Paciente'}`;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white dark:bg-slate-900 shadow-2xl rounded-lg">
        <DialogHeader className="text-center pt-4 flex flex-col items-center">
          ...
          <DialogTitle className={`text-2xl font-bold gradient-text ${role === 'paciente' ? 'text-center' : ''}`}>{title}</DialogTitle>
          <DialogDescription className={`text-muted-foreground ${role === 'paciente' ? 'text-center' : ''}`}>{description}</DialogDescription>
        </DialogHeader>
