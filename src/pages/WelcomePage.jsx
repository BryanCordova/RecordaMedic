...
const WelcomePage = ({ onLogin }) => {
  ...
  const handleAuthSuccess = (userOrAction) => {
    ...
    if (userOrAction.openLogin || userOrAction.openRegister) {
      ...
      const newMode = `${userOrAction.openLogin ? 'login' : 'register'}${userOrAction.role === 'admin' ? 'Admin' : 'Paciente'}`;
      setAuthMode(newMode);
    } else {
  ...
  const renderAuthModal = () => {
    ...
    const role = authMode.includes('Admin') ? 'admin' : 'paciente';
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-sky-100 via-indigo-50 to-purple-100 dark:from-slate-900 dark:via-indigo-950 dark:to-purple-950 text-gray-800 dark:text-gray-200">
      <header className="py-6 px-4 sm:px-8 flex justify-between items-center">
        ...
        <div className="space-x-2">
          <Button variant="outline" onClick={() => setAuthMode('loginPaciente')} className="dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-700">Iniciar Sesión Paciente</Button>
          <Button className="bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-600/90 text-white" onClick={() => setAuthMode('registroPaciente')}>
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-4 text-center">
        ...
        >
          ...
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Button size="lg" className="text-lg px-8 py-6 bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-600/90 text-white shadow-lg" onClick={() => setAuthMode('registroPaciente')}>
