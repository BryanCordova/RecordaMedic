...
const Inventario = ({ userId, onDataChange }) => {
  ...
  useEffect(() => {
    // Notificar si hay medicamentos agotados o bajos
    const agotados = inventario.filter(item => item.cantidad === 0);
    const bajos = inventario.filter(item => item.cantidad > 0 && item.cantidad <= item.cantidadMinima);
    if (agotados.length > 0) {
      toast({
        title: "¡Medicamento agotado!",
        description: `El medicamento '${agotados[0].medicamentoNombre}' está agotado en el inventario.`,
        variant: "destructive"
      });
    } else if (bajos.length > 0) {
      toast({
        title: "Inventario bajo",
        description: `El medicamento '${bajos[0].medicamentoNombre}' tiene pocas unidades en inventario.`,
        variant: "warning"
      });
    }
  }, [inventario, toast]);
