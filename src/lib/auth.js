...
export const registerUser = ({ username, email, password, role }) => {
  ...
  const newUser = {
    ...
    profileImg: '/assets/user-unisex.png',
  };
...
export const getAllClientData = () => {
  const users = getUsers();
  return users
    .filter(user => user.role === 'paciente')
    .map(pacienteUser => {
      const medicamentosPaciente = getMedicamentos(pacienteUser.id);
      const recordatoriosPaciente = getRecordatorios(pacienteUser.id);
      const inventarioPaciente = getInventario(pacienteUser.id);
      const interaccionesPaciente = getInteracciones(pacienteUser.id); 
      const historialPaciente = getHistorial(pacienteUser.id);
      
      return {
        ...pacienteUser,
        medicamentos: medicamentosPaciente,
        recordatorios: recordatoriosPaciente,
        inventario: inventarioPaciente,
        interacciones: interaccionesPaciente, 
...
export const updateUserClientDataInList = (clientId, updatedClientDataObject) => {
  const users = getUsers();
  const clientIndex = users.findIndex(user => user.id === clientId && user.role === 'paciente');
...
export const deleteUserClient = (userIdToDelete) => {
  let users = getUsers();
  const userExists = users.some(user => user.id === userIdToDelete && user.role === 'paciente');

  if (!userExists) {
    throw new Error("Usuario paciente no encontrado para eliminar.");
  }
